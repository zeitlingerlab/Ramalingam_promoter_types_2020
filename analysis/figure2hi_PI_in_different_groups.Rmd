...
options(knitr.figure_dir = "figure2hi_PI_in_different_groups")
...
late_by_group.df <- readRDS("other_analysis/late_wide.df.rds")
...
PI_at_late_genes.df %<>% merge(late_by_group.df)

PI_at_late_genes.df<-PI_at_late_genes.df[complete.cases(PI_at_late_genes.df),]

tg<-data.frame(above_bg_tissues=c(0,1,2,3,4,5,6),tissue_group=c('0','1_2','1_2',"3_4","3_4","5_6","5_6"))

PI_at_late_genes.df %<>% merge(tg)
...
saveRDS(PI_at_late_genes.df, file=figure_path("PI_at_late_genes.df.rds"))
...
ggplot(PI_at_late_genes.df,aes(y=PI,x=tissue_group, fill = tissue_group))+
...
theme(panel.grid.minor=element_blank(),
	  ...
	  ggplot(PI_at_late_genes.df,aes(x=PI,color=as.factor(tissue_group)))+
	  stat_ecdf()+
	  theme_bw()+
	  theme(panel.grid.minor=element_blank(),
